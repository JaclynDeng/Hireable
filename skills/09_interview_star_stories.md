# Step 09 Interview STAR Stories｜STAR 故事库

> **What**：把真实经历结构化为可复用的 STAR 故事库，按题型与面试官类型灵活调用
>
> **When**：行为面试前 / 需要「失败案例」「数据驱动」等特定题型的故事时
>
> **前置**：无（建议在 Skill 08 与 Skill 10 之间完成）
>
...
- 针对某家公司或某个岗位，将已有故事做定制化改写
- 面试中收到「失败案例」「影响力」「数据驱动」等特定题型，调取相应故事
- 反思面试后的表现，分析哪些故事说得好/不好

**行为面试的核心逻辑：** 面试官问的是「你过去做过什么」，背后评估的是「你能做什么」。所以每个故事都需要：
1. 有清晰的背景和你的具体角色
2. 有你做了什么（行动），而不是「团队做了什么」
3. 有可量化或可感知的结果
4. 有对当前岗位的迁移性表达（可选，但加分）

**本 Skill 不覆盖：**
- 自我介绍 / 离职原因 / 职业规划 → 参见 Skill 08（interview_hr）
- 业务判断类问题（「你如何看这个市场」）→ 参见 Skill 10（interview_manager）
...
核心原则：**故事不是背稿，而是从真实经历中找到「最有说服力的角度」。**
...
### 框架一：故事库构建（Story Inventory）

**目的：** 在面试前系统梳理可用的故事资产，按能力维度分类，确保不同题型都有对应故事。

**建议覆盖的 7 类能力维度：**

| 能力维度 | 适用题型 | 识别信号（从经历中找） |
|---------|---------|-------------------|
| 商业化/上市 | 「最有成就感的项目」「复杂项目管理」 | 新产品上市 / 0-1 项目 / 首次落地 |
| 数据分析/洞察 | 「如何用数据做决策」「数字化工具」 | 构建 Dashboard / 数据驱动改变决策 |
| 跨部门协作 | 「如何影响没有直接汇报关系的人」 | 需要协调多个职能才能完成的项目 |
| 客户洞察 | 「如何理解客户/市场需求」「VOC」 | 客户调研 / 反馈整合 / 需求转业务 |
| 创新/优先级 | 「如何在资源有限时做优先级决策」 | 从多个 idea 中筛选 / Innovation pipeline |
| 产品生命周期 | 「Portfolio 管理」「存量优化」 | 产品退市/升级/组合调整 |
| 失败/复盘 | 「说一个失败的经历」 | 跨部门推进卡壳 / 假设错误 / 执行延迟 |

**识别原则：**
- 优先选「你是主角（I led, I drove）」而非「团队做了」的故事
- 优先选「有量化结果」或「有显著影响」的经历
- 每个类别至少准备一个故事；王牌故事（如 0-1 上市）可以用于 2-3 类题目

---

### 框架二：STAR 结构化（Story Architecture）

**目的：** 将原始经历改写为面试官容易理解、听完有印象的结构化叙事。

**标准 STAR 格式（90秒版本）：**

```
S - Situation（背景）：1句话。什么项目/什么背景，量级感/独特性
T - Task（任务）：1句话。你的具体职责或目标，不是团队目标
A - Action（行动）：3-4条。你具体做了什么，用第一人称动词
R - Result（结果）：1-2句话。量化数字 + 业务影响
[迁移性表达（可选）]：1句话。这个经历与目标岗位的关联
```

**90秒分配建议：**
- S + T：15-20秒（快速建立背景，不要铺垫太多）
- A：40-50秒（这是核心，面试官最想听你怎么做的）
- R：15-20秒（量化 + 业务意义）
...
### 框架三：题型到故事的映射（Question Mapping）

**目的：** 在面试前建立清晰的「题型 → 调用故事」路由表，不在面试中临时搜索。
...
| 如何影响上级/无汇报关系的人 | 推动战略议题进入决策层 | influenced without authority / build case |

**使用同一故事回答不同题目的技巧：** 根据题目调整「行动层」的侧重点，而不是换一个全新故事：
- 「复杂协作」题 → 强调 stakeholder 识别与对齐过程
- 「商业化」题 → 强调 go-to-market 判断和结果
- 「数据」题 → 强调用了什么分析框架

---

### 框架四：受众适配（Audience Adaptation）

**目的：** 同一个故事，面对不同背景的面试官时，强调不同侧重点。

**三类面试官的关注重点：**

| 面试官类型 | 最关心的是 | 故事侧重调整 |
|----------|-----------|-----------|
| HR 初面 | 候选人是否稳定/靠谱/沟通清晰 | 逻辑清晰 + 强调协作和过程管理，不要太技术 |
| 直属经理（商业背景） | 候选人能否带来业务结果 | 强调客户价值、商业影响、市场机会识别 |
| 直属经理（技术/产品背景） | 候选人是否懂产品/技术约束 | 强调问题分解、技术挑战处理、跨部门对齐 |
| 销售 leader | 候选人能否支持销售成功 | 强调销售赋能、客户关系、商业落地 |

**具体调整方式：** 不改变 S + R，只在 A（行动）中改变 3 条里的 1-2 条侧重点：
- 面对商业背景 → 多说「识别了什么机会」「如何定义价值主张」
- 面对技术背景 → 多说「如何拆解问题」「如何和研发/供应链对齐」
- 面对销售背景 → 多说「如何让销售用起来」「客户反馈是什么」

---

### 框架五：失败案例专项（Failure Case Framework）
...
**失败案例的 5 步结构：**
```
1. Situation — 什么项目，你在其中的角色
2. Failure Point — 哪里没做好（具体的，不是模糊的）
3. Impact — 失败的实际影响（结果延迟/成本增加/客户不满）
4. Root Cause — 根因是什么（你的判断/行动层面，而不是「外部原因」）
5. What Changed — 你之后如何调整，并有具体成效
```

**选题原则：**
- 选「成熟型失败」：跨部门 alignment 失误 / 对市场假设过于自信 / 推进节奏判断错误
- 避免「低级型失败」：执行疏漏 / 数据输错 / 沟通不及时（这类失败传递的信号是「你不严谨」）
- 避免「假失败」：「我太完美主义了」「我接的任务太多了」（面试官能识别，会降低整体评分）
...
| 普通表达 | 升级表达 | 传递的能力信号 |
|--------|---------|-------------|
| 协调资源 | aligned cross-functional stakeholders around launch priorities | 跨部门影响力 |
| 做分析 | translated data into portfolio decisions | 数据 → 决策 的完整链路 |
| 整理反馈 | synthesized customer feedback into actionable insights | VOC 能力 |
| 推动上市 | led commercialization and launch readiness | 端对端所有权 |
| 做工具/报告 | built a scalable performance tracking tool that enabled decision-making | 工具的业务目的 |
| 跟销售沟通 | partnered with sales to align on segment priorities and enablement gaps | 合作关系的具体化 |
| 提建议 | developed a business case and influenced leadership to include it in strategy | 影响力的完整表达 |

**删掉内部术语：** 每家公司有自己的术语（KPI系统名/流程代码/项目代号），面试中用内部术语会让面试官无法理解。替换为通用描述：
- 内部项目代号 → 「a structured innovation program from ideation to commercialization」
- 系统名称 → 「a cross-functional performance tracking tool」
- 内部职称 → 「Product Portfolio Owner」「Global Commercialization Lead」

**结尾迁移句模板：**
> 「这段经历对我申请这个岗位的意义在于——[目标岗位]同样需要[能力X]，而我通过[故事核心动作]建立了这项能力的实战基础。」
